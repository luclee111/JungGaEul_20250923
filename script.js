...
//여기다가 점수 입력하면 댐
const parameters = [
  { name: '자립성', a: 3, b: 0, sub: '', hoverSub: '', iconUrl: './images/Palja_Param_Icon_Bigeop.png' },
  ...
  { name: '책임감', a: 1, b: 3, sub: '', hoverSub: '', iconUrl: './images/Palja_Param_Icon_Gwansung.png' },
  { name: '사고력', a: 0, b: 0, sub: '', hoverSub: '', iconUrl: './images/Palja_Param_Icon_Inseong.png' },
];
...
window.addEventListener('load', function() {
  // ❗ 모든 help-icon을 찾는다
  const helpIcons = document.querySelectorAll('.slider-help-icon'); // ✅ class로 여러개 찾기
  let videoTimeout = null;

  helpIcons.forEach((helpIcon, index) => {
    const guideOverlay = document.getElementById(`slider-nav-guide-${index + 1}`);
    const guideCloseBtn = document.getElementById(`slider-guide-close-${index + 1}`);
    const swiperContainer = guideOverlay.querySelector('.guide-swiper');

    // 각 guideSwiper 인스턴스를 만든다
    const guideSwiper = new Swiper(swiperContainer, {
      loop: false,
      centeredSlides: true,
      slidesPerView: 1,
      pagination: {
        el: guideOverlay.querySelector('.swiper-pagination'),
        clickable: true,
      },
      navigation: {
        nextEl: guideOverlay.querySelector('.swiper-button-next'),
        prevEl: guideOverlay.querySelector('.swiper-button-prev'),
      },
      breakpoints: {
        768: { slidesPerView: 1 },
    ...
    helpIcon.addEventListener('click', () => {
      ...
      guideSwiper.update(); // 슬라이더 강제 업데이트

      const allVideos = guideOverlay.querySelectorAll('.guide-video');
